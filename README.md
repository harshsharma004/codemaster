...
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/officialasishkumar/CodeMaster)

CodeMaster is a two-part workspace:
...
- Set `CodeMaster_SECRET_KEY` to a strong secret that is at least 32 characters long.
- Set `CodeMaster_ALLOWED_HOSTS` and `CodeMaster_CORS_ORIGINS` for the domains you actually serve.
- Keep `CodeMaster_SEED_DEMO_DATA=false` in production.
- Keep `CodeMaster_DATABASE_AUTO_MIGRATE=false` in production and run `go run ./cmd/CodeMaster migrate up` explicitly before rollout.
- Keep `CodeMaster_ENABLE_ADMIN=false` unless you also set `CodeMaster_ADMIN_EMAILS` explicitly.
