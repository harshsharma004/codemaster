...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  const [activeGroup, setActiveGroup] = useState<number | null>(() => {
    const stored = localStorage.getItem("CodeMaster-active-group");
  ...
  useEffect(() => {
    if (activeGroup) {
      localStorage.setItem("CodeMaster-active-group", String(activeGroup));
    } else {
      localStorage.removeItem("CodeMaster-active-group");
    }
  }, [activeGroup]);
