...
export const ProblemCard = ({ problem, index = 0, onClick, onDelete }: ProblemCardProps) => {
  return (
    <motion.button
      ...
      className="group flex w-full items-center gap-3.5 rounded-xl bg-card border-2 border-border p-3 text-left transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
      onClick={onClick}
      aria-label={`Open details for ${problem.title}`}
    >
      <div className="w-12 h-12 rounded-lg bg-secondary border border-border/50 flex items-center justify-center flex-shrink-0 overflow-hidden">
        <ProblemThumbnail title={problem.title} url={problem.url} thumbnailUrl={problem.thumbnailUrl} />
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-base font-black text-foreground truncate">{problem.title}</span>
        </div>
