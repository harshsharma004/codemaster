...
type BarState = "idle" | "comparing" | "shifting" | "sorted";
...
export default function InsertionSortViz() {
  ...
  const barColor = (state: BarState) => {
    switch (state) {
      ...
      case "shifting": return "bg-amber-400";
    ...
    <div className="flex flex-1 flex-col bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/5 via-background to-secondary/20 w-full">
      <div className="w-full max-w-5xl mx-auto px-6 pt-6 pb-16 space-y-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">Insertion Sort</h1>
            <p className="text-sm text-muted-foreground">Builds a sorted array one element at a time by comparing and shifting.</p>
          </div>
        </motion.div>
