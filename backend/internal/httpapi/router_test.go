...
import (
	...
	"CodeMaster/backend/internal/security"
)

func TestLoginRateLimitingAndSecurityHeaders(t *testing.T) {
	db := newTestDB(t)
	passwordHash, err := security.HashPassword("CodeMaster!123")
	if err != nil {
...
func TestAdminLoginUsesEmailField(t *testing.T) {
	...
	form := strings.NewReader("email=admin%40example.com&password=CodeMaster%21123")
...
func testSettings() config.Settings {
	return config.Settings{
		...
		SessionCookieName:               "CodeMaster-admin-session",
		Port:                            "8000",
	}
}
