...
import (
	...
	"CodeMaster/backend/internal/database"
	"CodeMaster/backend/internal/httpapi"
	"CodeMaster/backend/internal/migrations"
	"CodeMaster/backend/internal/ratelimit"
	"CodeMaster/backend/internal/search"
	"CodeMaster/backend/internal/startup"
)
...
func RunServer(ctx context.Context, logger *slog.Logger, settings config.Settings) error {
	...
	if settings.RunStartupTasksOnAppStart {
		...
		logger.Info("startup tasks disabled; search indices will remain cold until the service is restarted with CodeMaster_RUN_STARTUP_TASKS_ON_APP_START=true")
	}
